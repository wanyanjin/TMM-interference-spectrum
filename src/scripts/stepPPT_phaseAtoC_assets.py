...
def build_baseline_assets() -> None:
    ...
    render_structure_schematic(schematic)
    render_baseline_curve(frame, curve)
    copy_alias(schematic, slide.main_png)
...
- 直接复用图：无
- 基于已有数据重绘：
  - `baseline_structure_schematic.png`：新绘制的 PPT 结构示意图
...
def build_summary_assets() -> None:
    ...
    copy_alias(matrix, slide.secondary_png)
    write_text(
        slide.text_md,
        """
# 标题建议
