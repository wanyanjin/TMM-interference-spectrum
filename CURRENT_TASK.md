...
目标：在现有 `pvk_x01` 审计结果基础上生成 HTML slide deck 与 technical report，用于解释反射率校准、TMM 计算链和当前实验-理论偏差；并为下一步 `nk_audit` / `nk_envelope` 做准备。

## 本轮范围

- 生成 Phase 08 反射率校准与 TMM 审计 HTML slide deck
...
1. 新增 `step08_build_audit_slide_deck.py`
2. 生成 `phase08_reference_audit_deck.html` 与 `technical_report.html`
3. 为下一步 `nk_audit` 与 `nk_envelope` 保留可讲解的审计材料
...
- HTML 中不得出现绝对路径或外部 CDN。
- deck 只讲 `pvk_x01` audited 结果集，不引入新的重算分支。

## 立即下一步

- 生成 HTML slide deck
- 生成 `technical_report.html`
